...
*/
    ...
    import { changeRelogDelay } from './funcs/changeRelogDelay.js';
    import { displayChat } from './funcs/displayChat.js';
...
log4js.configure({
  ...
  logChatSystem('Logger initialized: ' + JSON.stringify(logger))
...
logChatSystem(chalk.hex('#55FF55')('Starting timer...'))
function startMinecraft() {
    ...
    bot.on('kicked', function(reason) {
        ...
        logChatSystem("Kicked from server. ", reason)
    });
    ...
    bot.on('end', function(reason) {
        ...
        if(config.autoRelog){
        sendDiscordMessage('Bot disconnected, relogging in ' + config.relogDelay + 'seconds.');
        setTimeout(function() {
            startMinecraft();
        }, config.relogDelay * 1000) //seconds * 1000 = miliseconds
                }
    });
...
function sendDiscordMessage(message) {
    ...
    const req = https.request(config['discord']['webhook'], options, res => {
      ...
      logChatSystem(`statusCode: ${res.statusCode}`)
      res.on('data', d => {
...
client.once(discordjs.Events.ClientReady, c => {
    ...
    logChatSystem(chalk.hex('#FFFF55')(`Ready! Logged in as ${c.user.tag}`))
});
...
if (message.content.startsWith('!viewChat')) {
  displayChat(message);
    ...
    if (message.content.startsWith('!send')) {
        ...
        console.log(text); 
        logChatSystem('User input via !send: ' + text)
        bot.chat(text);
        setTimeout(function() {
          displayChat(message);
        }, 1000);
      ...
      if (message.content.startsWith('!relogDelay')) {
        changeRelogDelay(message);
    }
      ...
      if (message.content.startsWith('!setVersion')) {
        ...
        logChatSystem('Changed version to ' + vtext)
  ...
  if (matchesAddItem2) {
    ...
    logChatSystem('webhook 1 added:' + webhook1Add)
    ...
    logChatSystem(chalk.hex('#55FFFF')(`Set Webhook1 to **` + config.discord.webhook + '**'))
  ...
  if (matchesAddItem5) {
    ...
    logChatSystem(chalk.hex('#55FFFF')(`Set Server IP to **` + config.serverip + '**'))
