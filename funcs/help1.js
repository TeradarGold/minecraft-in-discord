...
function displayHelpPage1(message) {
   let helpSetServer = '!setServer <ip>                                        Sets the server IP to <ip> so you can !connect to a different server\n\n';
    ...
    let helpAutoRelog = "!autoRelog                                           Toggles automatically relogging if disconnected from server. Currently " + config.autoRelog + "\n\n";
    ...
    message.reply('```' + helpSetServer + helpSend + helpViewChat + helpInventory + helpAutoRelog + helpConnect + helpDisconnect + helpSetVersion + helpWebhook1 + '```');
  }
